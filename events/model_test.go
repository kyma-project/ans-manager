...
func JSONEq(t *testing.T, s string, s2 string) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.expectedError != "" {
				...
				require.NoError(t, err)
				JSONEq(t, testCase.expected, string(testCaseJSON))
			}
		})
	}
...
func Test_NotificationMapping(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, string(nmJSON), "typeKey")
	require.Contains(t, string(nmJSON), "recipient1")
	require.Contains(t, string(nmJSON), "user1")

	// Test with options
	nm2 := NewNotificationMapping("typeKey2", *recipients, WithDeduplicationID("dedupID"), WithNotificationTemplateKey("templateKey"))
	require.NoError(t, nm.Validate())
	require.Equal(t, "dedupID", nm2.DeduplicationID)
	require.Equal(t, "templateKey", nm2.NotificationTemplateKey)
